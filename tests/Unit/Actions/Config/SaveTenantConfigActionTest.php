...
final class SaveTenantConfigActionTest extends TestCase
{
    public function test_saves_tenant_config_by_merging_with_existing_data(): void
    {
        ...
        $this->mockService(GetTenantFilePathAction::class, function ($mock) use ($configPath): void {
            ...
            $mock->shouldReceive('execute')->andReturnUsing(static fn (string $name): string => $configPath);
        });
        ...
        $this->mockService(SaveArrayAction::class, function ($mock) use (&$savedData, &$savedPath, $configPath): void {
            ...
            $mock->shouldReceive('execute')->andReturnUsing(
                static function (mixed $data, mixed $filename) use (&$savedData, &$savedPath, $configPath): bool {
