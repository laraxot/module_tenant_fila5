...
> **Nota 2026-09-17**: `TenantContract` sotto non è stato trovato in `app/Contracts/` (che contiene solo
> `SushiToJsonContract`/`SushiToJsonsContract`). Verificare ogni contratto/servizio citato qui contro il
> codice prima di usarlo — vedi il banner di verifica in [architecture.md](./architecture.md).
